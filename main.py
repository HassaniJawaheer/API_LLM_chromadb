...
#En page:
@app_proj.get("/presentation")
async def home():
    text = str("Tu dois créer une api, avec fastapi,qui prend en entrée une"+  
...
async def uptdate_template(new_model: Parameters):
    try:
...
def upload(file: UploadFile = File(...)):
    if file.content_type == "text/plain":
        ...
        try:
           documents = DirectoryLoader("", glob='Textpdfversion.pdf', loader_cls=PyPDFLoader)
...
async def register_question(query: str):
    ...
    response = da.answering(query, chunks_db, store_parameters[0].key_openai,
